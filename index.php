...
function pageProfile(?array $user): void {
    ...
    // Equipo favorito (el más apostado)
    $stmt = $db->prepare("SELECT team, COUNT(*) as cnt FROM bets WHERE user_id=? GROUP BY team ORDER BY cnt DESC LIMIT 1");
    $stmt->execute([$user['id']]);
    $favTeam = $stmt->fetch();

    // Minuto favorito
    $stmt = $db->prepare("SELECT minute, COUNT(*) as cnt FROM bets WHERE user_id=? GROUP BY minute ORDER BY cnt DESC LIMIT 1");
    $stmt->execute([$user['id']]);
    $favMinute = $stmt->fetch();

    // Última apuesta ganada
    $stmt = $db->prepare("SELECT b.*, m.home_team, m.away_team FROM bets b JOIN matches m ON b.match_id=m.id WHERE b.user_id=? AND b.status='won' ORDER BY b.created_at DESC LIMIT 1");
    $stmt->execute([$user['id']]);
    $lastWin = $stmt->fetch();

    $winRate = $stats['total_bets'] > 0 ? round(($stats['won'] / $stats['total_bets']) * 100) : 0;
    $profit  = (float)$stats['total_won'] - (float)$stats['total_wagered'];

    // Avatares disponibles
    $avatars = ['⚽','🏆','🥅','👟','🦅','🔥','⭐','🎯','🦁','👑','🐉','⚡','🌟','🎪','🏅'];

    // Manejo de cambio de avatar
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'change_avatar') {
        csrfVerify();
        $newAvatar = trim($_POST['avatar'] ?? '');
        if (in_array($newAvatar, $avatars, true)) {
            $db->prepare("UPDATE users SET avatar=? WHERE id=?")->execute([$newAvatar, $user['id']]);
            flash('success', 'Avatar actualizado ✅');
        }
        redirect('/index.php?page=profile');
    }
?>
<div class="page-wrap" style="max-width:800px">
  ...
  <p class="page-subtitle">Tu historial y estadísticas</p>

  <!-- Tarjeta de perfil -->
  <div class="card mb-3" style="background:linear-gradient(135deg,var(--bg2) 0%,rgba(26,58,107,0.3) 60%,rgba(201,168,76,0.05) 100%);border-color:rgba(201,168,76,0.2)">
    <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">
      <div style="position:relative">
        <div style="width:80px;height:80px;background:var(--bg3);border:2px solid rgba(201,168,76,0.3);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:42px">
          <?= h($user['avatar']) ?>
        </div>
      </div>
      <div style="flex:1;min-width:0">
        ...
        <div style="margin-top:8px">
          <?php if ($winRate >= 50): ?>
            <span class="badge badge-won">🔥 <?= $winRate ?>% win rate</span>
          <?php elseif ($winRate > 0): ?>
            <span class="badge badge-pending">🎯 <?= $winRate ?>% win rate</span>
          <?php else: ?>
            <span class="badge badge-closed">⚽ Sin apuestas aún</span>
          <?php endif; ?>
          <?php if ($lastWin): ?>
            <span class="badge badge-won" style="margin-left:6px">🏆 Último ganador</span>
          <?php endif; ?>
        </div>
      </div>
      <div style="text-align:right">
        <div style="font-family:var(--font-head);font-size:32px;color:var(--gold);text-shadow:var(--glow-gold)"><?= formatCedenas((float)$user['balance']) ?></div>
        <div style="font-size:11px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px">Saldo actual</div>
      </div>
    </div>
  </div>
  ...
  <div class="grid-3 mb-3">
    ...
    <div class="stat-box">
      <div class="stat-value text-green"><?= (int)$stats['won'] ?></div>
      <div class="stat-label">Ganadas</div>
    </div>
    <div class="stat-box">
      <div class="stat-value" style="color:<?= $winRate>=50?'var(--green)':'var(--red)' ?>"><?= $winRate ?>%</div>
      <div class="stat-label">Win Rate</div>
    </div>
    <div class="stat-box">
      <div class="stat-value text-gold"><?= formatCedenas((float)$stats['total_won']) ?></div>
    ...
    <div class="stat-box">
      <div class="stat-value text-muted"><?= formatCedenas((float)$stats['total_wagered']) ?></div>
      <div class="stat-label">Total apostado</div>
    </div>
    <div class="stat-box">
      <div class="stat-value" style="color:<?= $profit>=0?'var(--green)':'var(--red)' ?>"><?= ($profit>=0?'+':'').formatCedenas($profit) ?></div>
  ...
  <div class="grid-2 mb-3">
    <!-- Datos curiosos -->
    <div class="card">
      <div class="card-header">Tus datos curiosos</div>
      <div class="flex-between mb-2 fs-sm">
        <span class="text-muted">Equipo favorito</span>
        <span class="fw-bold font-sub"><?= $favTeam ? h($favTeam['team']).' ('.$favTeam['cnt'].'x)' : '—' ?></span>
      </div>
      <div class="flex-between mb-2 fs-sm">
        <span class="text-muted">Minuto favorito</span>
        <span class="fw-bold font-sub"><?= $favMinute ? 'Min '.$favMinute['minute'].' ('.$favMinute['cnt'].'x)' : '—' ?></span>
      </div>
      <div class="flex-between mb-2 fs-sm">
        <span class="text-muted">Apuestas pendientes</span>
        <span class="fw-bold font-sub text-gold"><?= (int)$stats['pending'] ?></span>
      </div>
      <?php if ($lastWin): ?>
      <div class="flex-between fs-sm">
        <span class="text-muted">Último premio</span>
        <span class="fw-bold font-sub text-green"><?= formatCedenas((float)$lastWin['prize_cedenas']) ?></span>
      </div>
      <?php endif; ?>
    </div>
    ...
    <div class="card">
      ...
      <form method="POST" action="/index.php?page=profile">
        ...
        <input type="hidden" name="avatar" id="selectedAvatar" value="<?= h($user['avatar']) ?>">
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:16px">
          <?php foreach ($avatars as $av): ?>
          <button type="button"
            onclick="document.getElementById('selectedAvatar').value='<?= $av ?>'; document.querySelectorAll('.av-btn').forEach(b=>b.style.background='var(--bg3)'); this.style.background='rgba(201,168,76,0.2)'; this.style.borderColor='var(--gold)';"
            class="av-btn"
            style="aspect-ratio:1;font-size:24px;background:<?= $av===$user['avatar']?'rgba(201,168,76,0.2)':'var(--bg3)' ?>;border:1px solid <?= $av===$user['avatar']?'var(--gold)':'var(--border)' ?>;border-radius:8px;cursor:pointer;transition:all 0.15s">
            <?= $av ?>
          </button>
          <?php endforeach; ?>
        </div>
        <button type="submit" class="btn btn-primary btn-block">Guardar Avatar</button>
      </form>
    </div>
  </div>
  ...
  <div class="flex gap-2" style="flex-wrap:wrap">
    <a href="/index.php?page=my_bets" class="btn btn-ghost">🎯 Ver mis apuestas</a>
    <a href="/index.php?page=wallet"  class="btn btn-ghost">💰 Ver wallet</a>
    <a href="/index.php?page=matches" class="btn btn-primary">⚽ Apostar ahora</a>
  </div>
</div>
<?php }
...
function pageProfile(?array $user): void {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) FROM recharge_requests WHERE user_id=? AND status='approved'"); $stmt->execute([$user['id']]); $totalRecharges = (int)$stmt->fetchColumn();
...
<div class="page-wrap" style="max-width:800px">
    ...
    <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="margin-top:6px">
          <span class="badge <?= $user['role']==='admin'?'badge-live':'badge-open' ?>">
            <?= $user['role']==='admin'?'👑 Admin':'⚽ Jugador' ?>
          </span>
          <span class="badge badge-pending" style="margin-left:6px">
            Desde <?= date('M Y', strtotime($user['created_at'])) ?>
          </span>
        </div>
      </div>
      <div style="text-align:right">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px">
    ...
    <div class="stat-box">
      <div class="stat-value text-green"><?= $wonBets ?></div>
      <div class="stat-label">Ganadas</div>
    </div>
    <div class="stat-box">
      <div class="stat-value" style="font-size:26px"><?= $winRate ?>%</div>
      <div class="stat-label">Win Rate</div>
    </div>
    <div class="stat-box">
      <div class="stat-value text-gold" style="font-size:22px"><?= formatCedenas($totalWon) ?></div>
      <div class="stat-label">Total Ganado</div>
    </div>
    <div class="stat-box">
      <div class="stat-value" style="font-size:22px"><?= formatCedenas($totalBet) ?></div>
      <div class="stat-label">Total Apostado</div>
    </div>
    <div class="stat-box">
      <div class="stat-value <?= $roi>=0?'text-green':'text-red' ?>" style="font-size:26px"><?= $roi>=0?'+':'' ?><?= $roi ?>%</div>
        ...
        <form method="POST" action="/index.php?page=profile">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:4px">
              ...
              <label style="cursor:pointer">
                <input type="radio" name="avatar" value="<?= h($av) ?>" <?= $user['avatar']===$av?'checked':'' ?> style="display:none">
                <div class="avatar-option" style="width:42px;height:42px;background:var(--bg3);border:2px solid <?= $user['avatar']===$av?'var(--gold)':'var(--border)' ?>;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:22px;transition:all 0.15s" onclick="selectAvatar(this, '<?= h($av) ?>')">
                  <?= $av ?>
                </div>
              </label>
              <?php endforeach; ?>
    ...
    <div class="card">
      ...
      <?php if (empty($recentBets)): ?>
        <div class="empty-state" style="padding:24px">
          <div class="icon">🎯</div>
          <h3>Sin apuestas aún</h3>
          <a href="/index.php?page=matches" class="btn btn-primary btn-sm mt-2">Apostar ahora</a>
        </div>
      <?php else: ?>
        <?php foreach ($recentBets as $b): ?>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border)">
          ...
          <div style="text-align:right">
            <?php if ($b['status']==='won'): ?>
              <span class="badge badge-won">🏆 +<?= formatCedenas((float)$b['prize_cedenas']) ?></span>
            <?php elseif ($b['status']==='lost'): ?>
              <span class="badge badge-lost">❌ -<?= formatCedenas((float)$b['amount_cedenas']) ?></span>
            <?php else: ?>
              <span class="badge badge-pending">⏳ <?= formatCedenas((float)$b['amount_cedenas']) ?></span>
            <?php endif; ?>
          </div>
        </div>
        <?php endforeach; ?>
...
<script>
function selectAvatar(el, val) {
  document.querySelectorAll('.avatar-option').forEach(a => a.style.borderColor = 'var(--border)');
  el.style.borderColor = 'var(--gold)';
...
// =============================================
// PÁGINA DE RANKING Y ESTADÍSTICAS
// =============================================
